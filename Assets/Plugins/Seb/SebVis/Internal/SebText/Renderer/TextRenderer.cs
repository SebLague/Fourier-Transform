...
namespace Seb.Vis.Text.Rendering
{
    public class TextRenderer
    {
        ...
        public void RenderTest(CommandBuffer cmd)
        {
            // Create buffers
            ComputeHelper.CreateArgsBuffer(ref argsBuffer, quadMesh, perGlyphInstanceData.Count);
            ComputeHelper.CreateStructuredBuffer(ref instanceDataBuffer, perGlyphInstanceData);
            ComputeHelper.CreateStructuredBuffer(ref bezierBuffer, textRenderData.bezierPoints);
            ComputeHelper.CreateStructuredBuffer(ref metadataBuffer, textRenderData.glyphMetadata);
            ComputeHelper.CreateStructuredBuffer(ref textGroupBuffer, textGroups);
        ...
        public static BoundingBox CalculateLocalBounds(ReadOnlySpan<char> text, FontData fontData, LayoutSettings settings)
        {
            return CreateTextLayout(text, fontData, settings);
        }
        ...
        static BoundingBox CreateTextLayout(ReadOnlySpan<char> text, FontData fontData, LayoutSettings settings)
        {
            return CreateTextLayout(text, fontData, settings, Color.white, false, null, null, 0);
        }
        ...
        static BoundingBox CreateTextLayout(ReadOnlySpan<char> text, FontData fontData, LayoutSettings settings, Color textCol, bool populateInstances, List<InstanceData> instances, TextRenderData renderData, int groupIndex)
        {
            ...
            for (int i = 0; i < text.Length; i++)
            {
                ...
                else if (info.type is LayoutHelper.ChunkType.ColorBlockEnd)
                {
                    i += info.richTextIndexJump;
                    currCol = textCol;
                    continue;
        ...
        public void Release()
        {
            ...
            if (fontMat != null)
            {
                if (Application.isPlaying) UnityEngine.Object.Destroy(fontMat);
                else UnityEngine.Object.DestroyImmediate(fontMat);
            }
        }
        ...
        public struct LayoutSettings : IEquatable<LayoutSettings>
        {
            ...
            public static LayoutSettings CreateDefault(float fontSize)
            {
                return new(fontSize, 1, 1, 1);
            }
        }
