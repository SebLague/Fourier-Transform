...
public class MyVisTest : MonoBehaviour
{
    ...
    void DrawUI()
    {
        ...
        {
            if (UI.DrawButton(text, UI.TestButtonTheme, buttonPos, buttonSize, true, true, buttonAnchor))
            {
                Debug.Log("Pressed");
            }
    ...
    void DrawLotsOfPoints()
    {
        ...
        for (int i = 0; i < pointTestCount; i++)
        {
            ...
            Seb.Vis.Draw.Point(pos, rad, col);
        }
    }
    ...
    void DrawSceneObjects()
    {
        ...
        for (int i = 0; i < transform.childCount; i++)
        {
            ...
            if (t.TryGetComponent(out PointVisTest p))
            {
                ...
                Seb.Vis.Draw.Ellipse(p.transform.position, (Vector2)(p.transform.localScale), p.Col);
            }
            else if (t.TryGetComponent(out QuadVisTest q))
            {
                if (q.outline) Seb.Vis.Draw.QuadOutline(q.transform.position, q.transform.localScale, q.thickness, q.col);
                else Seb.Vis.Draw.Quad(q.transform.position, q.transform.localScale, q.col);
            }
            else if (t.TryGetComponent(out LineVisTest l))
            {
                Seb.Vis.Draw.Line(l.transform.position, l.transform.position + (Vector3)l.offset, l.thickness, l.col, l.t);
            }
            ...
            {
                Seb.Vis.Draw.Text(tv.fontType, tv.text, tv.fontSize, tv.transform.position, tv.textAlign, tv.col);
            }
            else if (t.TryGetComponent(out PathVisTest path))
            {
                Seb.Vis.Draw.LinePath(path.GetComponentsInChildren<Transform>().Select(t => (Vector2)t.position).ToArray(), path.thickness, path.col, path.t);
            }
            else if (t.TryGetComponent(out ArrowVisTest a))
            {
                if (a.useHeadDefaults)
                {
                    Seb.Vis.Draw.Arrow(a.transform.position, (Vector2)a.transform.position + a.offset, a.thickness, a.col, a.t);
                }
                else
                {
                    Seb.Vis.Draw.Arrow(a.transform.position, (Vector2)a.transform.position + a.offset, a.thickness, a.headLength, a.headAngle, a.col, a.t);
                }
            }
            else if (t.TryGetComponent(out TriangleVisTest tri))
            {
                Vector2 c = tri.transform.position;
                Seb.Vis.Draw.Triangle(c + tri.offsetA, c + tri.offsetB, c + tri.offsetC, tri.col);
            }
        }
